...
new             = zeros([1 28]);
new(1:12)       = [26 12 20 42 96 410 1147 4875 54304 1079551 22804439 434782476]; % ms
new_s           = new ./ 1000; % s
new_m           = new_s ./ 60; % min
new_u           = new_m ./ 60; % uur
new_d           = new_u ./ 24; % dag
...
set(gca, 'Yscale', 'log')
hold on
plot(2:13, new_u(1:12), 'o-')

%%
diff = (old_m(1:12) - new_m(1:12));
cumdiff = cumsum(diff);
...
T2 = table(xas(1:12), new(1:12));
figure
plot(xas(1:12),old(1:12),'o')
hold on
plot(xas(1:12),new(1:12),'o')
set(gca,'YScale','log')
[p, ~,mu ] = polyfit(T.Var1,  T.Var2,  11);
[p2,~,mu2] = polyfit(T2.Var1, T2.Var2, 11);
f  = polyval(p,  xas, [], mu);
...
%%
benadering_uur = f2(13) / (1000 * 60 * 60)
